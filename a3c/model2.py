...
class Agent(ModelBase):
    def __init__(self, input_size, act_space, config):
        ...
        self.add_param('policy_fc_last', (config.hidden_size_1, act_space))
        self.add_param('vf_fc_last', (act_space, 1))
        self.add_param('vf_fc_last_bias', (1,))
        self.num_epochs = 0.0001
        self.alpha = 5
    ...
    def forward(self, Xs):
        hs = np.maximum(0, np.matmul(Xs.reshape((*Xs.shape[:-2], -1)), self.params['fc1']))
        # hs = np.maximum(0, batch_dot(hs, self.params['fc2']))
        logits = np.matmul(hs, self.params['policy_fc_last'])
    ...
    def act(self, ps):
        ...
        rands = numpy.random.uniform(size=ps.shape)
        epsilon = numpy.random.beta(a = self.alpha, b = self.num_epochs, size=ps.shape[0])
        roll = numpy.random.uniform(size=ps.shape[0])
        as_ = ps.asnumpy().argmax(axis = 1) * (roll >= epsilon) + rands.argmax(axis = 1) * (roll < epsilon)
        return as_
